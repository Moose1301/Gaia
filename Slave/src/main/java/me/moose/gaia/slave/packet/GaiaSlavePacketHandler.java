...
import me.moose.gaia.common.packet.packets.slave.user.GaiaSlaveUserConsoleMessagePacket;
...
public class GaiaSlavePacketHandler implements IGaiaSlavePacketHandler {
    ...
    public void handle(GaiaMasterUserDataPacket.Data packet) {
        ...
        if(!profile.isCompletelyLoaded()) {
            ...
            GaiaServer.getLogger().debug("PacketHandler", "Completely Loaded " + profile.getUsername() + "'s Profile in " +
                    (System.currentTimeMillis() - profile.getStartLoaded()) + "ms", Logger.DebugType.SUCCESS);
        }
    }
    ...
    public void handle(GaiaMasterUserMessagePacket.ConsoleMessage packet) {
        System.out.println(packet.getUuid().toString() + " " + packet.getMessage());
    ...
    @Override
    public void handle(GaiaMasterSlaveRestartPacket packet) {
